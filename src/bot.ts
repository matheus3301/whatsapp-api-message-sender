...
const setup = async () => {
    ...
    fs.writeFile('cookies.json', JSON.stringify(response.cookies, null, 2), function (err) {
        if (err) throw err;
...
const bot = async () => {
    ...
    for(let message of messages){
        try {
            await page.goto(`https://web.whatsapp.com/send?phone=${message.to}&text=${encodeRFC5987ValueChars(message.message)}`, { waitUntil: "load" });
